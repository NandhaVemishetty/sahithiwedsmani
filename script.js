...
const eventData = {
  haldi: {
    ...
    time: '10:30 AM', venue: "Bride's House", image: 'assets/haldi.png', alt: 'Haldi ceremony illustration',
  ...
  mehendi: {
    ritual: 'Celebration 2', day: 'Friday, 14 August 2026', title: 'Mehendi Ceremony',
    time: '5:00 PM', venue: "Bride's House", image: 'assets/mehendi.png', alt: 'Mehendi ceremony illustration',
    theme: 'mehendi-theme', tagline: 'An evening of intricate designs, music and beautiful memories.',
    note: 'Come celebrate colour, laughter and the joy of togetherness.'
  },
  sangeeth: {
    ritual: 'Celebration 3', day: 'Saturday, 15 August 2026', title: 'Sangeeth Night',
    time: '6:00 PM', venue: 'SSVL A/C Function Hall, Mahabubabad', image: 'assets/sangeeth.png', alt: 'Sangeeth celebration illustration',
  ...
  wedding: {
    ritual: 'Celebration 4', day: 'Sunday, 16 August 2026', title: 'The Wedding',
...
musicToggle.addEventListener('click', () => {
  if (audio.paused) {
    audio.play().then(() => musicToggle.classList.add('playing')).catch(() => {
      alert('Add your licensed audio file as assets/pelli-pusthakam.mp3 to enable music.');
    });
  } else {
    audio.pause();
...
document.querySelectorAll('.side-btn').forEach(button => button.addEventListener('click', () => {
  document.querySelectorAll('.side-btn').forEach(item => item.classList.remove('selected'));
  button.classList.add('selected');
  document.getElementById('sideChoice').textContent = `You picked ${button.dataset.side}. Welcome to the celebration!`;
  launchPetals(16);
}));
...
document.getElementById('rsvpForm').addEventListener('submit', event => {
  ...
  const text = `Namaste, this is ${name}. We are delighted to attend ${selectedEvent} for Dr. Sahithi and Mani Krishna. Number of guests: ${count}.`;
  window.open(`https://wa.me/919440145384?text=${encodeURIComponent(text)}`, '_blank', 'noopener');
});

document.getElementById('calendarBtn').addEventListener('click', event => {
  event.preventDefault();
  const calendar = `BEGIN:VCALENDAR\nVERSION:2.0\nBEGIN:VEVENT\nDTSTART:20260816T051500Z\nDTEND:20260816T073000Z\nSUMMARY:Wedding of Dr. Sahithi & Mani Krishna\nLOCATION:Nandana A/C Gardens, Mahabubabad\nDESCRIPTION:Wedding ceremony at 10:45 AM. Lunch follows at 12:30 PM.\nEND:VEVENT\nEND:VCALENDAR`;
  const blob = new Blob([calendar], { type: 'text/calendar' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = 'Sahithi-Mani-Wedding.ics';
  link.click();
